...
public final class WeatherModulation {
    ...
    /**
     * Clears cached weather data so the next {@link #ensureFresh()} call re-fetches.
     * Does NOT touch coordinates ({@code latitude}, {@code longitude}, {@code cityName})
     * — those are persistent location state managed by {@link #initFromSettings()} and
     * {@link #assignRandomLocation()}.  Plugins that reset/modify anti-ban settings
     * (AntibanPlugin.startUp, onProfileChanged) call this method; zeroing coordinates
     * there would force a random reassignment, losing the saved location.
     */
